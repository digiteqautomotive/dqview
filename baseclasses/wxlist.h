...
public:
	...
	CBaseList(LPCTSTR pName,
              INT iItems);

	CBaseList(LPCTSTR pName);
#ifdef UNICODE
	CBaseList(LPCSTR pName,
              INT iItems);
...
protected:
    ...
    */
	...
	void *GetValidI(POSITION p) const;
...
public:
    ...
    /* Add the list *pList to *this after position p in *this
       ...
       Existing POSITIONs in *this are undisturbed, including p.
    */
	AddAfter(POSITION p, CBaseList *pList);
...
public:
	...
	OBJECT *GetValid(POSITION p) const { return (OBJECT *) GetValidI(p); }
