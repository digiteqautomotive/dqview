  ...
  ITypeInfo ** pptinfo)
{
    ...
    *pptinfo = NULL;
	...
	lpfnLoadRegTypeLib = (LPLOADREGTYPELIB)(void*)GetProcAddress(hInst,
							      szRegTypeLib);
	...
	if (FAILED(hr)) {
		...
		lpfnLoadTypeLib = (LPLOADTYPELIB)(void*)GetProcAddress(hInst, szTypeLib);
...
CMediaEvent::CMediaEvent(LPCTSTR name, LPUNKNOWN pUnk) :
    CUnknown(name, pUnk)
{
}
...
CMediaPosition::CMediaPosition(LPCTSTR name,
							   LPUNKNOWN pUnk,
							   HRESULT * phr) :
    CUnknown(name, pUnk)
{
...
CPosPassThru::CPosPassThru(LPCTSTR pName,
			   LPUNKNOWN pUnk,
			   HRESULT *phr,
			   IPin *pPin) :
...
STDMETHODIMP
CPosPassThru::ConvertTimeFormat(LONGLONG * pTarget,
...
STDMETHODIMP
CPosPassThru::SetPositions(LONGLONG * pCurrent,
                            DWORD CurrentFlags, 
							LONGLONG * pStop,
                            DWORD StopFlags )
{
...
CRendererPosPassThru::CRendererPosPassThru(LPCTSTR pName,
					   LPUNKNOWN pUnk,
					   HRESULT *phr,
					   IPin *pPin) :
...
CSourcePosition::CSourcePosition(LPCTSTR pName,
				 LPUNKNOWN pUnk,
				 HRESULT* phr,
				 CCritSec * pLock) :
    CMediaPosition(pName, pUnk),
...
CBasicAudio::CBasicAudio(LPCTSTR pName, LPUNKNOWN punk) :
    CUnknown(pName, punk)
{
}
...
CBaseVideoWindow::CBaseVideoWindow(LPCTSTR pName, LPUNKNOWN punk) :
    CUnknown(pName, punk)
{
}
...
CBaseBasicVideo::CBaseBasicVideo(LPCTSTR pName, LPUNKNOWN punk) :
    CUnknown(pName, punk)
{
}
...
CCmdQueue::CCmdQueue(HRESULT *phr) :
