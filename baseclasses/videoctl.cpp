...
LPTSTR WINAPI StringFromResource(LPTSTR pBuffer, int iResourceID)
{
...
#ifdef UNICODE
LPSTR WINAPI StringFromResource(LPSTR pBuffer, int iResourceID)
{
...
LPWSTR WINAPI WideStringFromResource(LPWSTR pBuffer, int iResourceID)
{
    *pBuffer = 0;
...
HRESULT CLoadDirectDraw::LoadDirectDraw(LPSTR szDevice)
{
	...
	pDrawCreate = (PDRAWCREATE)(void*)GetProcAddress(m_hDirectDraw,"DirectDrawCreate");
    // force ANSI, we assume it
	pDrawEnum = (PDRAWENUM)(void*)GetProcAddress(m_hDirectDraw,"DirectDrawEnumerateA");
	pDrawEnumEx = (LPDIRECTDRAWENUMERATEEXA)(void*)GetProcAddress(m_hDirectDraw,
