...
*/
CBaseList::CBaseList(LPCTSTR pName,    // Descriptive list name
...
#ifdef UNICODE
CBaseList::CBaseList(LPCSTR pName,    // Descriptive list name
                     INT iItems) :    // Node cache size
#ifdef DEBUG
    CBaseObject(pName),
#endif
...
void *CBaseList::GetValidI(POSITION p) const
{
    CNode * pn = (CNode *) p;
    void *pObject = pn->GetData();
    // ASSERT(pObject != NULL);    // NULL pointers in the list are allowed.
